...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      advanceWorkflow(leave, 'approve', payload.userId, payload.role, comment);
    ...
    const htmlBody = `
      ...
      <p>Your leave application has been <strong>approved</strong> by your ${payload.role.replace('_', ' ')}.</p>
      ${comment ? `<p><strong>Comment:</strong> ${comment}</p>` : ''}
    `;
    sendEmail(applicant.email, subject, htmlBody).catch((e: Error) => console.error('Silent email failure:', e.message));

    return NextResponse.json({ message: `Leave application approved successfully.`, leave: { status: leave.status, currentStage: leave.currentStage } });
  } catch (error: any) {
